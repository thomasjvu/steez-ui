/**
 * Standalone Steez UI copy of Loading Screen.
 *
 * Generated from packages/ui/src/components/LoadingScreen.tsx by pnpm registry:generate.
...
const __styles_steez_loading_screen_0: Record<string, string> = {
  "root": "steez-loading-screen-0-root",
  "barGroup": "steez-loading-screen-0-barGroup",
  "bar": "steez-loading-screen-0-bar",
  "barFilled": "steez-loading-screen-0-barFilled",
  "value": "steez-loading-screen-0-value",
};

const __styles_steez_loading_screen_1: Record<string, string> = {
  "screen": "steez-loading-screen-1-screen",
  "fullscreen": "steez-loading-screen-1-fullscreen",
  "contained": "steez-loading-screen-1-contained",
  "lightTheme": "steez-loading-screen-1-lightTheme",
  "darkTheme": "steez-loading-screen-1-darkTheme",
  "crossContainer": "steez-loading-screen-1-crossContainer",
  "crossSvg": "steez-loading-screen-1-crossSvg",
  "crossLanded": "steez-loading-screen-1-crossLanded",
  "crossShape": "steez-loading-screen-1-crossShape",
  "centerStack": "steez-loading-screen-1-centerStack",
  "logoWrap": "steez-loading-screen-1-logoWrap",
  "title": "steez-loading-screen-1-title",
  "progressRow": "steez-loading-screen-1-progressRow",
  "footerStack": "steez-loading-screen-1-footerStack",
  "footerBrand": "steez-loading-screen-1-footerBrand",
  "footerMessage": "steez-loading-screen-1-footerMessage",
};

const __steezStandaloneStyles = String.raw`/* packages/ui/src/components/LoadingProgressBar.module.css */
.steez-loading-screen-0-root {
  display: flex;
  align-items: center;
  gap: 4px;
}

.steez-loading-screen-0-barGroup {
  display: flex;
  width: 100%;
  max-width: 650px;
  flex-wrap: wrap;
...
.steez-loading-screen-0-barFilled {
  background: currentColor;
}

.steez-loading-screen-0-value {
  width: 4ch;
  margin-left: 16px;
  text-align: right;
...
/* packages/ui/src/components/LoadingScreen.module.css */
.steez-loading-screen-1-screen {
  position: fixed;
  inset: 0;
  z-index: 9999;
  display: flex;
...
.steez-loading-screen-1-fullscreen {
  position: fixed;
}

.steez-loading-screen-1-contained {
  position: absolute;
  border: 1px solid var(--border-color, rgba(203, 203, 204, 0.2));
}

.steez-loading-screen-1-lightTheme {
  background: #fafbfc;
  color: #1a1a1a;
}

.steez-loading-screen-1-darkTheme {
  background: #0a0a0a;
  color: #cbcbcc;
}

@keyframes steez-loading-screen-1-crossFall {
  0% {
...
.steez-loading-screen-1-crossContainer {
  position: absolute;
  top: 0;
  left: var(--cross-left, 0px);
  z-index: 1;
  will-change: transform;
  animation: steez-loading-screen-1-crossFall 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
  animation-delay: var(--cross-delay, 0ms);
}

.steez-loading-screen-1-crossSvg {
  transform: scale(var(--cross-scale, 1));
  transform-origin: center top;
  opacity: var(--cross-opacity, 0.6);
}

.steez-loading-screen-1-crossLanded .steez-loading-screen-1-crossSvg {
  opacity: 1 !important;
}

.steez-loading-screen-1-crossShape {
  color: currentColor;
}

.steez-loading-screen-1-centerStack {
  position: relative;
  z-index: 10;
  display: flex;
  flex-direction: column;
...
.steez-loading-screen-1-logoWrap {
  display: flex;
  align-items: center;
  justify-content: center;
}

.steez-loading-screen-1-title {
  font-size: clamp(24px, 8vw, 48px);
  font-weight: 700;
  letter-spacing: 8px;
}

.steez-loading-screen-1-progressRow {
  position: relative;
  z-index: 10;
}

.steez-loading-screen-1-footerStack {
  position: absolute;
  right: 40px;
  bottom: 40px;
  z-index: 10;
...
.steez-loading-screen-1-footerBrand {
  display: flex;
  align-items: center;
  justify-content: flex-end;
}

.steez-loading-screen-1-footerMessage {
  opacity: 0.5;
  font-family: var(--font-mono, "Maple Mono", "SF Mono", "Cascadia Code", "Fira Code", Monaco, monospace);
  font-size: 14px;
}
...
`;
const __steezStandaloneStyleKey = "loading-screen";
...
export function LoadingProgressBar({
  ...
  return (
    <div
      className={className ? `${__styles_steez_loading_screen_0.root} ${className}` : __styles_steez_loading_screen_0.root}
      role="progressbar"
      aria-valuemin={0}
      aria-valuemax={100}
    ...
    >
      <div className={__styles_steez_loading_screen_0.barGroup}>
        {BAR_INDEXES.map((index) => {
          const isFilled = index < filledBars;
          return (
            <div
              key={index}
              className={`${__styles_steez_loading_screen_0.bar} ${isFilled ? __styles_steez_loading_screen_0.barFilled : ""}`.trim()}
              style={{ ["--bar-delay" as string]: `${index * 20}ms` }}
            />
          );
        })}
      </div>
      <div className={__styles_steez_loading_screen_0.value}>{displayLabel}</div>
    </div>
  );
}
...
export function LoadingScreen({
  ...
  return (
    <div
      ref={containerRef}
      className={`${__styles_steez_loading_screen_1.screen} ${fullscreen ? __styles_steez_loading_screen_1.fullscreen : __styles_steez_loading_screen_1.contained} ${
        isLightMode ? __styles_steez_loading_screen_1.lightTheme : __styles_steez_loading_screen_1.darkTheme
      }`.trim()}
      style={
        {
    ...
    >
      {crosses.map((cross) => (
        <div
          key={cross.id}
          className={__styles_steez_loading_screen_1.crossContainer}
          style={
            {
          ...
          onAnimationEnd={(event) => {
            if (event.animationName === "crossFall") {
              event.currentTarget.classList.add(__styles_steez_loading_screen_1.crossLanded);
            }
          }}
        >
          <div className={__styles_steez_loading_screen_1.crossSvg}>
            <LightCrossIcon className={__styles_steez_loading_screen_1.crossShape} />
          </div>
        </div>
      ))}

      <div className={__styles_steez_loading_screen_1.centerStack}>
        {logo ? <div className={__styles_steez_loading_screen_1.logoWrap}>{logo}</div> : null}
        <div className={__styles_steez_loading_screen_1.title}>{title}</div>
      </div>

      <LoadingProgressBar progress={displayProgress} className={__styles_steez_loading_screen_1.progressRow} />

      <div className={__styles_steez_loading_screen_1.footerStack}>
        {footerBrand ? (
          <div className={__styles_steez_loading_screen_1.footerBrand}>{footerBrand}</div>
        ) : null}
        <div className={__styles_steez_loading_screen_1.footerMessage}>
          {cleanMessage}
          {ellipsis}
        </div>
      </div>
